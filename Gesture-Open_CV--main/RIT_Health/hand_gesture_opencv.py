...
class HandGesture:
    def __init__(self, history_len=10):
        ...
        try:
            ...
            try:
                ...
                raise ImportError("Could not find a compatible MediaPipe Hands API. Please install a MediaPipe version that provides mediapipe.tasks.python.vision or mediapipe.solutions.")
        ...
        if self.task_api:
            ...
            if not os.path.exists(model_path):
                try:
                    ...
                    raise RuntimeError(f"Failed to download MediaPipe hand landmarker model: {e}")
            self.hands = self.HandLandmarker.create_from_model_path(model_path)
        else:
    ...
    def detect_gesture(self, frame):
        ...
        if self.task_api:
            mp_image = self.Image(self.ImageFormat.SRGB, frame_rgb)
